...
class LiteLLMProvider:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        logger.info(
            f"Initialized LiteLLM provider: {self.provider} with model: {self.model_name}"
        )
    ...
    def _get_model_name(self, model_name: Optional[str]) -> str:
        ...
        if model_name:
            ...
            elif self.provider == "openrouter" and not model_name.startswith(
                "openrouter/"
            ):
    ...
    def complete(self, messages: List[Dict[str, Any]], **kwargs) -> str:
        ...
        try:
                ...
                if (
                    ...
                    processed_message["content"] = [
                        {"type": "text", "text": message["content"]["text"]},
                        {
                            "type": "image_url",
                            "image_url": {
                                "url": f"data:image/png;base64,{message['content']['image']}"
                            },
                        },
                    ]
                else:
            ...
            logger.debug(
                f"Making LiteLLM completion call with model: {self.model_name}"
            )
    ...
    async def acomplete(self, messages: List[Dict[str, Any]], **kwargs) -> str:
        ...
        try:
            ...
            logger.debug(
                f"Making async LiteLLM completion call with model: {self.model_name}"
            )
        ...
        try:
            ...
            for message in messages:
                if isinstance(message, SystemMessage):
                    ...
                    # Handle multimodal content (text + images) for vision models
            ...
            # Call LiteLLM
            response = self.litellm_provider.complete(
                litellm_messages, **completion_kwargs, **kwargs
            )
    ...
    async def _agenerate(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            # Call async LiteLLM
            response = await self.litellm_provider.acomplete(
                litellm_messages, **completion_kwargs, **kwargs
            )

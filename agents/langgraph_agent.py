...
file_handler = logging.FileHandler(
    f"logs/pokemon_agent_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
)
...
class PokemonAgentState(BaseModel):
    ...
    game_state: Optional[Dict] = Field(
        default=None, description="Current game state information"
    )

    # Memory components
    short_term_memory: List[Dict] = Field(
        default_factory=list,
        description="Recent events and observations (last ~20 steps)",
    )

    # Task tracking
    current_task: Optional[str] = Field(
        default=None, description="The current task/goal the agent is working on"
    )
    task_history: List[str] = Field(
        default_factory=list, description="History of completed tasks"
    )

    # Map/navigation data
    known_locations: Dict[str, Any] = Field(
        default_factory=dict, description="Information about discovered locations"
    )

    # Current action
    action: Optional[Dict] = Field(default=None, description="Action to take")
    action_reasoning: Optional[str] = Field(
        default=None, description="Reasoning behind the action"
    )

    # Current visual analysis
    current_visual_analysis: str = Field(
        default="", description="Visual analysis from the current step"
    )

    # Execution tracking
    step_count: int = Field(default=0, description="Number of steps taken")
    last_error: Optional[str] = Field(default=None, description="Last error message")
    consecutive_llm_errors: int = Field(
        default=0, description="Count of consecutive LLM call errors"
    )

    # Trace for debugging
    trace: List[Dict] = Field(
        default_factory=list, description="Trace of agent actions and reasoning"
    )
    ...
    def get_relevant_short_term_memories(
...
class PokemonServerInterface:
    ...
    def initialize(
        ...
        try:
            ...
            logger.info(
                f"Initialization successful, location: {self.current_state['location']}"
            )
    ...
    def take_action(self, action_type: str, **kwargs) -> Dict[str, Any]:
        ...
        if not self.initialized:
            raise RuntimeError(
                "Environment not initialized, please call initialize() first"
            )
...
class PokemonSingleAgent:
    ...
    def __init__(
        ...
        self._explicit_session_id = (
            session_id  # Track if session was explicitly provided
        )
        self.session_id = session_id or datetime.datetime.now().strftime(
            "%Y%m%d_%H%M%S"
        )
        ...
        # Log file for thoughts (streaming-friendly)
        self.thoughts_log_file = os.path.join(
            log_dir, f"agent_thoughts_{self.session_id}.log"
        )
        ...
        logger.info(
            f"Pokemon Single Agent initialized with session ID: {self.session_id}, log level: {log_level}"
        )
...
5. Possible interactions available""",
                    ...
                    response = self.llm_provider.invoke(
                        [SystemMessage(content=self._get_agent_prompt()), human_message]
                    )
            ...
            if location and location != "Unknown":
                if location not in state.pokemon_state.known_locations:
                    ...
                    state.pokemon_state.known_locations[location]["last_visit"] = (
                        state.pokemon_state.step_count
                    )
        ...
        def construct_prompt(state: AgentState) -> AgentState:
            ...
            # Add to messages
            state.messages.append({"role": "human", "content": human_message})
            ...
            max_recent = min(
                8, len(recent_non_system)
            )  # Keep at most 8 non-system messages
            ...
            if len(state.messages) > 10:
                logger.info(
                    f"Trimmed message history to {len(state.messages)} messages"
                )
        ...
        def think(state: AgentState) -> AgentState:
            ...
            logger.info(
                f"  Short-term memories: {len(state.pokemon_state.short_term_memory)}"
            )
            ...
            while retry_count <= max_retries:
                try:
                    ...
                    if retry_count > 0:
                        logger.warning(
                            f"Retry attempt {retry_count}/{max_retries} for LLM call"
                        )
                    ...
                    # Write the LLM response to both streaming file and persistent log
                    try:
                        location = (
                            state.pokemon_state.game_state.get("location", "Unknown")
                            if state.pokemon_state.game_state
                            else "Unknown"
                        )
                        ...
                        with open(self.thoughts_log_file, "a", encoding="utf-8") as f:
                            f.write(
                                f"\n[{timestamp}] === Step {state.pokemon_state.step_count} at {location} ===\n"
                            )
                            f.write(ai_message)
                            f.write(f"\n{'-' * 80}\n")
                        ...
                        logger.error(
                            f"Failed to write streaming thoughts: {streaming_error}"
                        )
                    ...
                    if retry_count <= max_retries:
                        ...
                        backoff_time = retry_backoff[
                            min(retry_count - 1, len(retry_backoff) - 1)
                        ]
                        logger.info(
                            f"Waiting {backoff_time}s before retry {retry_count}"
                        )
                        ...
                        state.pokemon_state.last_error = (
                            f"LLM call failed after {max_retries} retries: {error_msg}"
                        )
                        ...
                        if state.pokemon_state.consecutive_llm_errors >= 3:
                            ...
                            logger.warning(
                                "Multiple consecutive LLM failures - trying recovery action"
                            )
                            directions = ["up", "down", "left", "right"]
                            recovery_button = random.choice(directions)
                            state.pokemon_state.action = {
                                "action_type": ActionType.PRESS_KEY,
                                "button": recovery_button,
                            }
                            logger.info(
                                f"RECOVERY ACTION: Press {recovery_button} button"
                            )
                        else:
                            # Default safe action
                            state.pokemon_state.action = {
            ...
            if ai_message:
                ...
                if result.get("memory_updates"):
                    logger.info(
                        f"MEMORY UPDATES: {len(result.get('memory_updates', []))} new memories identified"
                    )
                ...
                state.pokemon_state.add_to_trace(
                    "decide", f"Decision: {result.get('action', {})}"
                )
            else:
                # This should not happen, but just in case
                logger.error(
                    "Unexpected error: No AI message after successful LLM call"
                )
                state.pokemon_state.consecutive_llm_errors += 1
                state.pokemon_state.last_error = (
                    "No AI message after successful LLM call"
                )
                state.pokemon_state.action = {
                    "action_type": ActionType.WAIT,
                    "frames": 10,
                ...
                state.pokemon_state.add_to_trace(
                    "error", "No AI message after successful LLM call"
                )
        ...
        def execute_action(state: AgentState) -> AgentState:
            ...
            try:
                ...
                if dialog:
                    logger.info(
                        f'GAME STATE: At {location}, Dialog: "{dialog[:50]}{"..." if len(dialog) > 50 else ""}"'
                    )
                else:
                ...
                state.pokemon_state.add_to_trace(
                    "error", f"Error executing action: {error_msg}"
                )
        ...
        def should_continue(state: AgentState) -> str:
            ...
            if state.recursion_depth > 1:
                logger.info(
                    f"Completed one full step cycle (depth={state.recursion_depth}), returning to main loop"
                )
        ...
        graph.add_conditional_edges(
            "execute", should_continue, {"continue": "observe", "stop": END}
        )
    ...
    def _extract_key_information(self, game_state: Dict) -> Dict:
            ...
            party_info.append(
                {
                    ...
                    "hp": {
                        ...
                        "max": hp.get("max", 0),
                        "percentage": int(
                            (hp.get("current", 0) / hp.get("max", 1)) * 100
                        )
                        if hp.get("max", 0) > 0
                        else 0,
                    },
        ...
        for item in inventory:
            inv_list.append(
                {"item": item.get("item", ""), "quantity": item.get("quantity", 0)}
            )
        key_info["inventory"] = inv_list

        # Add memory stats
        key_info["memory_stats"] = {
            "short_term_size": len(self.state.short_term_memory)
            if hasattr(self, "state") and hasattr(self.state, "short_term_memory")
            else 0
        }

        # Check if in battle
        key_info["in_battle"] = "battle" in game_state.get(
            "location", ""
        ).lower() or bool(game_state.get("in_battle", False))
    ...
    def _create_thinking_prompt(self, state: PokemonAgentState) -> str:
        ...
        dialog = game_state.get("dialog", "")
        if dialog:
            prompt_parts.extend(["", "# Dialog", dialog])

        # Add Pokemon party
        pokemons = game_state.get("pokemons", [])
        if pokemons:
            prompt_parts.extend(["", "# Pokemon Party"])
            ...
            for i, pokemon in enumerate(pokemons):
                ...
                if moves:
                    ...
                    for move in moves:
                        pp = move.get("pp", {})
                        move_texts.append(
                            f"{move.get('name', 'Unknown')} ({pp.get('current', 0)}/{pp.get('max', 0)})"
                        )
        ...
        inventory = game_state.get("inventory", [])
        if inventory:
            prompt_parts.extend(["", "# Inventory"])

            for item in inventory:
                prompt_parts.append(
                    f"- {item.get('item', 'Unknown')} x{item.get('quantity', 0)}"
                )

        # Add current task
        if state.current_task:
            prompt_parts.extend(["", "# Current Task (IMPORTANT)", state.current_task])

        # Add short-term memory (recent events)
        if state.short_term_memory:
            prompt_parts.extend(["", "# Recent History (Short-term Memory)"])
        ...
        if state.known_locations:
            prompt_parts.extend(["", "# Known Locations"])

            for location, info in state.known_locations.items():
                prompt_parts.append(
                    f"- {location} (visited {info.get('visit_count', 1)} times)"
                )
    ...
    def _parse_thinking_and_action(self, response: str) -> Dict:
        ...
        for line in lines:
            ...
            if line_lower.startswith("memory update") or line_lower.startswith(
                "memory updates"
            ):
                in_observe_section = False
                in_think_section = False
            ...
            if in_memory_section:
                if line.strip() and not line.strip().startswith("#"):
                    ...
                    if (
                        "important" in line_lower
                        or "remember" in line_lower
                    ...
                    memory_updates.append(
                        {"type": memory_type, "content": line.strip()}
                    )
        ...
        logger.info(
            f"PARSING RESULT: Action={action_str}, "
    ...
    def _log_thinking(
        self, state: PokemonAgentState, prompt: str, response: str, parsed_result: Dict
    ) -> None:
        """
    ...
    def _log_execution(
        ...
        log_entry = {
            ...
            if result
            else state.game_state.get("location", "Unknown"),
            "raw_response": thinking_data.get(
                "raw_response", ""
        ...
        visual_analysis = (
            state.current_visual_analysis
            if hasattr(state, "current_visual_analysis")
            else ""
        )
        ...
        if error:
            ...
            memory_entry["content"] = f"Error occurred: {error[:100]}" + (
                "..." if len(error) > 100 else ""
            )
        elif state.last_error:
            # In case of LLM errors that were handled earlier
            memory_entry["error_details"] = state.last_error
            memory_entry["content"] = (
                f"LLM error occurred: {state.last_error[:100]}"
                + ("..." if len(state.last_error) > 100 else "")
            )
        else:
            ...
            if dialog:
                content_parts.append(
                    f"Dialog: {dialog[:50]}{'...' if len(dialog) > 50 else ''}"
                )
            if visual_analysis:
                content_parts.append(
                    f"Visual: {visual_analysis[:50]}{'...' if len(visual_analysis) > 50 else ''}"
                )
            ...
            if not state.short_term_memory:
                ...
                for i, memory in enumerate(reversed(state.short_term_memory[-5:])):
                    ...
                    if mem_type == "consolidated":
                        ...
                        observe_summary = (
                            memory.get("observe", "")[:50] + "..."
                            if len(memory.get("observe", "")) > 50
                            else memory.get("observe", "")
                        )
                        ...
                        logger.info(
                            f"  {i + 1}. Step {step} at {location}: Observed: {observe_summary} | Action: {action}{visual_info}"
                        )
                    else:
    ...
    def run(self, max_steps: int = 1000000) -> None:
        ...
        """
        logger.info(
            f"===== Starting PokemonSingleAgent run with max_steps={max_steps} ====="
        )
        ...
        try:
            ...
            while step < max_steps and self.pokemon_server.running:
                ...
                logger.info(
                    f"Step {self.state.step_count} completed in {step_time:.2f} seconds"
                )
            ...
            logger.info(
                f"===== Run completed after {self.state.step_count} steps ====="
            )
            # Stop the server
            try:
    ...
    def _log_statistics(self):
        ...
        steps_per_hour = (
            int(self.state.step_count / (elapsed / 3600)) if elapsed > 0 else 0
        )
        ...
        if self.stats["locations_visited"]:
            top_locations = sorted(
                self.stats["locations_visited"].items(),
                key=lambda x: x[1],
                reverse=True,
            )[:3]
            locations_str = ", ".join(
        ...
        if self.stats["action_counts"]["press_key"]:
            button_stats = []
            for button, count in sorted(
                self.stats["action_counts"]["press_key"].items(),
                key=lambda x: x[1],
                reverse=True,
            ):
...
def main():
    ...
    parser.add_argument(
        "--temperature", type=float, default=1, help="Temperature for LLM generation"
    )
    ...
    parser.add_argument(
        "--load_state", type=str, default=None, help="Load a saved state file"
    )
    parser.add_argument(
        "--load_autosave", action="store_true", help="Load the latest autosave"
    )

    # Run settings
    parser.add_argument(
        "--max_steps", type=int, default=1000000, help="Maximum number of steps to run"
    )
    parser.add_argument(
        "--session_id",
        type=str,
